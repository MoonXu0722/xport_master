...
inline void write_log(const char *str, size_t size) {
...
#define SO_OPENTYPE 0x7008
  ...
  if (getaddrinfo(host, service.c_str(), &hints, &result)) {
	...
	//std::cout << "service.c_str()=" << service.c_str() << "\n";
  ...
  if (n > 0) {
    if (n >= bufsiz - 1) {
      ...
      write(&glowable_buf[0], n);
		...
		printf("%s", buf);
		//std::cout << "thread id3 " << std::this_thread::get_id() << "\n";
		//detail::write_log(buf, n);
      write(buf, n);
    }
  }
}
...
inline int SocketStream::read(char *ptr, size_t size) {
  if (detail::select_read(sock_, CPPHTTPLIB_READ_TIMEOUT_SECOND,
    ...
    int n = recv(sock_, ptr, static_cast<int>(size), 0);  
	//printf("rcv:[%d]%s\n", n, ptr);
    return n;
  }
  return -1;
...
inline int SocketStream::write(const char *ptr, size_t size) {
	int n = send(sock_, ptr, static_cast<int>(size), 0);
	//printf("send[%d] %s", n, ptr);
  return n;
}
...
inline bool Server::handle_file_request(Request &req, Response &res) {
	printf("base_dirr:%s[%d]\n", base_dir_.c_str(), base_dir_.empty());
...
inline bool Server::process_request(Stream &strm, bool last_connection,
  ...
  req.set_header("REMOTE_ADDR", strm.get_remote_addr().c_str());
	//std::cout << "remoteaddr:" << strm.get_remote_addr() << "\n";
  // Body
